function exportModifiedFlow(flowData, includeImages = true, saveToFile = true) {
    if (flowData) {
      let cleanFlowData = { ...flowData };
      console.log(cleanFlowData);
      cleanFlowData.screenshots = cleanFlowData.screenshots.map((s) => ({
        ...s,
        dataUrl: undefined,
      }));
      ...
      if (includeImages) {
        ...
        cleanFlowData.screenshots.forEach((screenshot) => {
          ...
          var fileName = `${Math.floor(date.getTime() / 1000)}_${
        ...
        var dataStr =
          ...
          encodeURIComponent(JSON.stringify(cleanFlowData));
